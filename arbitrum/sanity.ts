...
const arbitrumMulticall = new Multicall({
  ethersProvider: arbitrumProvider,
  tryAggregate: true,
});
...
const run = async () => {
  ...
  const mirrorLootOwners =
    {} || // temp solution for seeding
    aggregateLootOwners(
      await Promise.all(
        chunk(LOOT_IDS, 1000).map((thisChunk) =>
          getOwnersForLootIds(arbitrumMulticall, thisChunk, LOOT_MIRROR_ADDRESS)
        )
      )
    );
  ...
  sendOwnerUpdates(arbitrumProvider, ownerUpdates);
};
